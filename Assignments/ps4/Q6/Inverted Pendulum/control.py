...
# Initialize the value function, transition probabilities and reward function
value_func = 0.10 * np.random.rand(NUM_STATES, 1)
Psa = 1 / NUM_STATES * np.ones((NUM_STATES, NUM_ACTIONS, NUM_STATES))
reward_func = np.zeros((NUM_STATES, 1))

# Record the counts of observed trials to efficiently update the estimated state transition probabilities (Psa)
# transition_counts[i,j,k] is the total number of times of transitions from state s_i to state s_k using action a = j
transition_counts = np.zeros((NUM_STATES, NUM_ACTIONS, NUM_STATES))

# Record the observed rewards to efficiently update the reward function
# rewards_counts[i, 0] is the total reward value observed at state s_i
# rewards_counts[i, 1] is the total number of times the system transitions to state s_i
# But I don't know the reason of doing this, it seems to me that the reward function is fixed...
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    # Choose the optimal action with respect to the current value function
    # If the two actions give the same expected reward, randomly choose one by flipping a fair coin
    score = np.dot(Psa[state,:,:], value_func)
    if score[0] > score[1]:
        action = 0
    ...
    if new_state == NUM_STATES - 1:
        ...
        # mask_1 and mask_2 are Boolean indexing to choose the states (and actions) that have some observations
        total = np.sum(transition_counts, axis = 2)
        mask_1 = (total > 0)
        Psa[mask_1] = transition_counts[mask_1] / (total[mask_1].reshape(-1, 1))
        
        mask_2 = (reward_counts[:, 1] > 0)
        reward_func[mask_2] = (reward_counts[:, 0][mask_2] / reward_counts[:, 1][mask_2]).reshape(-1, 1)
        ...
        # Set up a counter to count the number of iterations required for convergence of value iteration
        count = 0
        
        # Value iteration
        while True:
            
            # Formula for value iteration
            # np.dot() computes dot product between 1-D array and 3-D array along the last axis
            # See https://docs.scipy.org/doc/numpy-1.15.0/reference/generated/numpy.dot.html
            updated_value_func = reward_func + GAMMA * np.max(np.dot(Psa, value_func), axis = 1)
            
            # Compute the max-norm (sup-norm) of difference between original value function and updated value fucntion
            max_norm = max(abs(updated_value_func - value_func))
            
            # Update the value function
            value_func = updated_value_func
            
            # Finish one iteration of value iteration
            count += 1
            
            # Check the convergence criterion using the above max-norm
            if max_norm < TOLERANCE:
                break
        
        if count == 1:
            # Value iteration converges in the first iteration
            consecutive_no_learning_trials += 1
        else:
            # Reset the consecutive_no_learning_trials variable, if it takes more than one iteration for value iteration to converge
            consecutive_no_learning_trials = 0
